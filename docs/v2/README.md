...
- **`M2_2A` is a "not run", not a failure.** Its verdict line reads FAIL by
  the gate's own binary rule, but the live half never executed for want of a
  credential; nothing in it measures the memory boundary failing.
